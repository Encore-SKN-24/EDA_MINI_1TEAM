...
본 분석을 통해 도출된 지역별 산업 전력 소비의 핵심 특징과 통계적 시사점은 다음과 같습니다.

### 1. 강력한 전력 수요 견인 변수 (Sensitivity Analysis)
기온과 습도는 산업 공정의 핵심 변수로 작동하며, 특정 지역에서 폭발적인 증폭 효과를 보입니다.
* **충청도 평균기온 (exp: 11.06)**: 기온이 1단위 변화할 때 전력 사용량이 **약 11.1배의 강도**로 매우 민감하게 반응합니다. 이는 반도체 클린룸 등 외부 온도에 극도로 민감한 **초정밀 항온항습 공정**이 밀집된 지역적 특성이 반영된 결과입니다.
* **경상남도 평균수증기압 (exp: 8.05)**: 수증기압이 1단위 증가할 때 전력 사용량이 **약 8.1배의 강도**로 급증합니다. 이는 기온보다 '습도'가 전력 수요를 더 강력하게 견인함을 의미하며, 산업용 공조 시스템이 공기 중 수분을 제거하는 **제습(잠열 제거) 과정**에 막대한 에너지를 소모하고 있음을 입증합니다.

### 2. 계수(coef) 부호에 따른 전력 소비 패턴의 통계적 입증
계수의 부호는 기준점 대비 전력 사용량의 상대적 증감을 나타내는 중요한 지표입니다.
* **음수(-) 계수의 의미**: 강원도와 충청도의 '월(Month)' 변수 계수가 음수로 나타나는 것은, 전력 소비가 극에 달하는 **1월(기준점)** 대비 타 계절의 조업 환경이나 난방 부하가 완화되어 사용량이 줄어드는 패턴을 통계적으로 보여줍니다.
* **지면온도의 반전 현상**: 지면온도가 **반비례(-, exp 1미만)**로 나타나는 충청도(0.137)와 경상남도(0.063)는 겨울철 차가운 지표면으로 인한 **공장 바닥 및 기초부 열 손실 방지**를 위한 히팅 부하가 산업용 전력 소비의 숨은 복병임을 나타냅니다.
