...
## 3. EDA & 데이터분석 방향성 
0. **전력 사용량과 온도 데이터의 추세 확인**: 전력 사용량과 평균 온도의 전반적인 추세 파악 
1. **계약종별 전력사용량 확인**: 계약종별(주택·일반·산업용등) 중 가장 큰 전력 소비 주체 파악
2. **상관관계 탐색**: 기온, 기압, 습도 등 기상 변수와 전력량의 변수간 연관 관계 분석
...
### 1. 대한민국 전체 지역의 연도 별 전력사용량 합계 추이
...
* **전력 사용량과의 상관관계**: '전력사용량 추이' 그래프와 비교했을 때, 최고 기온이 발생하는 시점과 전력 사용량 피크 시점이 일치합니다. 이는 **기온 상승이 직접적으로 전력 수요 급증을 유발**한다고 가정해 볼 수 있음
...
## 지역 & 핵심 계약종별 전력 사용량과 기상요인 상관관계 분석
<table width="100%">
  <tr>
    <td width="50%">
...
## 📊 통계 분석 개요 (GLM)
전력 소비량은 기온 등 기상 변수와 냉·난방 수요의 결합으로 인해 비선형적인 패턴(U자형)을 보입니다. 본 분석은 다음과 같은 방법론을 적용함

* **Gamma 분포 가정**: 전력 소비량은 0 이상의 연속형 변수이며 비대칭적인 분포(Tail Distribution)를 가집니다. 이를 정규분포로 가정하는 선형 회귀보다 **Gamma 회귀 모델**이 산업 공정의 냉각·가열 효율 변화를 분석하는 데 적합함
* **비선형성 고려**: 기온의 제곱항 및 냉·난방도일(HDD, CDD) 변수를 포함하여 날씨와 전력 간의 복합적인 관계를 모델링함
