...
## 1. 프로젝트 개요
   - 본 프로젝트는 국내 시/도 단위 전력 사용량 및 기상 데이터를 활용하여, 기후 요인이 전력사용량 변화에 미치는 정량적 영향과 비대칭적 패턴을 탐색하고 파악하는데 목적을 두고 있음.
   - 기간 : 2026.01.30 ~ 2025.02.04

## 2. 프로젝트 배경 및 필요성
- 2-1. 폭염·한파 시 냉·난방 전력 수요량이 매년 상승하고 있음
   <img width="800" height="500" alt="image" src="https://github.com/user-attachments/assets/f58ca700-7cc8-4b74-be1b-0795fae75459" />

- 2-2.기온과 전력사용량의 연관성은 사례를 통해 확인된다. 2025년 10월 제주의 날씨는 기온이 떨어지지 않아, 10월 들어 6일간 제주지역 하루 최대전력량이 800MW를 넘어섰다. 이는 기온이 전력 사용량에 큰 영향을 주는 것이라고 판단된다 *출처 : 제주의소리(https://www.jejusori.net)

## 3. 프로젝트 목적
1. 계약종별 전력소비량 확인: 가동 주체별(가정·일반·산업용 등) 중 어떤 전력사용량 비중을 가장 큰 계약종별은 무엇인가?  
2. 전력소비량과 기상 변수의 상관관계 확인: 날씨데이터(기온, 기압, 습도, 풍속등)은 전력 사용량과 어떤 상관관계를 갖는가?
3. 전력사용량에 미치는 날씨 요소 확인 : 비선형적인 날씨 데이터와 전력사용량 관계에서 어떤 변수들이 직접적으로 미치는 영향은 어떤 요소인가?
...
## 💡 수치 해석 가이드
1.  **계수(coef)가 음수(-)**: 수치가 커질수록 전력 사용량은 감소합니다. (예: 강원도 HDD가 낮아질수록 전력 소비 하락)
2.  **계수(coef)가 양수(+)**: 수치가 커질수록 전력 사용량은 증가합니다. (예: 충청도 기온이 높아질수록 전력 소비 급상승)
3.  **지수화(exp) 값**: 해당 변수가 1단위 변화할 때 전력 소비량이 몇 배 증가/감소하는지를 나타냅니다.
    * 충청도 기온(exp: 11.06): 기온 변화가 산업용 냉방 부하에 매우 강력한 증폭 효과를 줌을 시사합니다.
